// This file is @generated by prost-build.
/// See <https://docs.temporal.io/docs/concepts/queries/>
#[cfg_attr(
    feature = "serde_serialize",
    derive(::serde::Serialize, ::serde::Deserialize)
)]
#[derive(Clone, PartialEq, ::prost::Message)]
pub struct WorkflowQuery {
    /// The workflow-author-defined identifier of the query. Typically a function name.
    #[prost(string, tag = "1")]
    pub query_type: ::prost::alloc::string::String,
    /// Serialized arguments that will be provided to the query handler.
    #[prost(message, optional, tag = "2")]
    pub query_args: ::core::option::Option<super::super::common::v1::Payloads>,
    /// Headers that were passed by the caller of the query and copied by temporal
    /// server into the workflow task.
    #[prost(message, optional, tag = "3")]
    pub header: ::core::option::Option<super::super::common::v1::Header>,
}
impl ::prost::Name for WorkflowQuery {
    const NAME: &'static str = "WorkflowQuery";
    const PACKAGE: &'static str = "temporal.api.query.v1";
    fn full_name() -> ::prost::alloc::string::String {
        "temporal.api.query.v1.WorkflowQuery".into()
    }
    fn type_url() -> ::prost::alloc::string::String {
        "/temporal.api.query.v1.WorkflowQuery".into()
    }
}
/// Answer to a `WorkflowQuery`
#[cfg_attr(
    feature = "serde_serialize",
    derive(::serde::Serialize, ::serde::Deserialize)
)]
#[derive(Clone, PartialEq, ::prost::Message)]
pub struct WorkflowQueryResult {
    /// Did the query succeed or fail?
    #[prost(enumeration = "super::super::enums::v1::QueryResultType", tag = "1")]
    pub result_type: i32,
    /// Set when the query succeeds with the results.
    /// Mutually exclusive with `error_message` and `failure`.
    #[prost(message, optional, tag = "2")]
    pub answer: ::core::option::Option<super::super::common::v1::Payloads>,
    /// Mutually exclusive with `answer`. Set when the query fails.
    /// See also the newer `failure` field.
    #[prost(string, tag = "3")]
    pub error_message: ::prost::alloc::string::String,
    /// The full reason for this query failure. This field is newer than `error_message` and can be encoded by the SDK's
    /// failure converter to support E2E encryption of messages and stack traces.
    /// Mutually exclusive with `answer`. Set when the query fails.
    #[prost(message, optional, tag = "4")]
    pub failure: ::core::option::Option<super::super::failure::v1::Failure>,
}
impl ::prost::Name for WorkflowQueryResult {
    const NAME: &'static str = "WorkflowQueryResult";
    const PACKAGE: &'static str = "temporal.api.query.v1";
    fn full_name() -> ::prost::alloc::string::String {
        "temporal.api.query.v1.WorkflowQueryResult".into()
    }
    fn type_url() -> ::prost::alloc::string::String {
        "/temporal.api.query.v1.WorkflowQueryResult".into()
    }
}
#[cfg_attr(
    feature = "serde_serialize",
    derive(::serde::Serialize, ::serde::Deserialize)
)]
#[derive(Clone, Copy, PartialEq, Eq, Hash, ::prost::Message)]
pub struct QueryRejected {
    #[prost(enumeration = "super::super::enums::v1::WorkflowExecutionStatus", tag = "1")]
    pub status: i32,
}
impl ::prost::Name for QueryRejected {
    const NAME: &'static str = "QueryRejected";
    const PACKAGE: &'static str = "temporal.api.query.v1";
    fn full_name() -> ::prost::alloc::string::String {
        "temporal.api.query.v1.QueryRejected".into()
    }
    fn type_url() -> ::prost::alloc::string::String {
        "/temporal.api.query.v1.QueryRejected".into()
    }
}
